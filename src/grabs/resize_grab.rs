...
impl TryFrom<xdg_toplevel::ResizeEdge> for ResizeEdge {
    type Error = ();

    #[inline]
    fn try_from(x: xdg_toplevel::ResizeEdge) -> Result<Self, Self::Error> {
        Self::from_bits(x as u32).ok_or(())
    }
}
...
impl PointerGrab<Smallvil> for ResizeSurfaceGrab {
    ...
    fn motion(
        ...
        if self.edges.intersects(ResizeEdge::LEFT | ResizeEdge::RIGHT) {
            ...
            new_window_width = f64_to_i32_saturating(f64::from(self.initial_rect.size.w) + delta.x);
        }
        ...
        if self.edges.intersects(ResizeEdge::TOP | ResizeEdge::BOTTOM) {
            ...
            new_window_height =
                f64_to_i32_saturating(f64::from(self.initial_rect.size.h) + delta.y);
        }
...
impl ResizeSurfaceState {
    ...
    const fn commit(&mut self) -> Option<(ResizeEdge, Rectangle<i32, Logical>)> {
        match *self {
